...
async function Listen(ID) {
    ...
    let  { seconds, minutes, golds } = await cxListen(ID); // 查询可领取的时间

    if (seconds.length === 0 && minutes.length === 0  && golds.length === 0) {
        $.log(`🟠 没有可领取的听歌时间，任务终止`);
        return;
    }
    ...
    for (let listenTime of seconds) {
        await performListenTask(loginUid, loginSid, listenTime, listenGold, "秒");
    }

    // **然后领取分钟任务**
    for (let listenTime of minutes) {
        await performListenTask(loginUid, loginSid, listenTime,listenGold, "分钟");
    }

    //接口
    for (let listenGold of golds) {
        await performListenTask(loginUid, loginSid, listenTime, listenGold, "积分");
    }
}

// 📌 执行听歌任务
async function performListenTask(loginUid, loginSid, listenTime, listenGold, unitType) {
    let gold = ListenGold
    $.log(`🟡 正在使用 ${listenTime} 时间 进行听歌任务`);
    $.log(`🟡 正在使用 ${listenGold} 接口 进行听歌任务`);

    let options = {
        url: `https://integralapi.kuwo.cn/api/v1/online/sign/v1/earningSignIn/newDoListen?loginUid=${loginUid}&loginSid=${loginSid}&from=listen&goldNum=${ListenGold}&listenTime=${listenTime}&unit=${unitType}`,
        headers: kw_headers,
    };
    console.log(`${url}`)
    ...
    await $.http.get(options).then((resp) => {
        ...
        if (obj.code === 200 && obj.success && obj.data.description === 1) {
            desc = obj.data.description || '成功';
            desc = `🎉 每日听歌成功: ${desc}（使用 ${listenTime} ${unitType}）`;
        } else if (obj.msg === "今天已完成任务") {
            desc = `🟢 每日听歌: ${obj.data.description}（使用 ${listenTime} ${unitType}）`;
        } else if (obj.msg === "用户未登录") {
            desc = `🔴 每日听歌: ${obj.data.description}（使用 ${listenTime} ${unitType}）`;
        } else {
            desc = `⚠️ 每日听歌: ${obj.data.description}（使用 ${listenTime} ${unitType}）`;
        }
